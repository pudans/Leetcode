...
class Medium198 : ArraysTopic, DynamicProgrammingTopic {
    ...
    fun rob(nums: IntArray): Int {
        ...
        if (nums.size == 2) return maxOf(nums.first(), nums[1])
        val dp = IntArray(nums.size)
        dp[0] = nums.first()
        dp[1] = maxOf(nums.first(), nums[1])
    ...
    fun rob1(nums: IntArray): Int {
        if (nums.size == 1) return nums.first()
        if (nums.size == 2) return Math.max(nums.first(), nums[1])
        nums[2] += nums.first()
        for (i in 3 until nums.size) {
            nums[i] += Math.max(nums[i - 2], nums[i - 3])
        }
        println(nums.toList())
        return Math.max(nums.last(), nums[nums.lastIndex - 1])
    }
}
